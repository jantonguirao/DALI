...
bool SelectMasksCPU::SetupImpl(std::vector<OutputDesc> &output_desc,
  ...
  DALI_ENFORCE(in_mask_meta_shape.sample_dim() == 2, "``mask_meta`` input is expected to be 2D");
  ...
  DALI_ENFORCE(in_mask_coords_shape.sample_dim() == 2,
               "``mask_coords`` input is expected to be 2D");
  ...
  output_desc[0].shape = in_mask_meta_shape;
  output_desc[0].type = in_mask_meta.type();
  auto &out_mask_meta_shape = output_desc[0].shape;
  output_desc[1].shape = in_mask_coords_shape;
  output_desc[1].type = in_mask_coords.type();
  auto &out_mask_coords_shape = output_desc[1].shape;

  samples_meta_.resize(nsamples);
  for (int i = 0; i < nsamples; i++) {
    auto &meta = samples_meta_[i];
    meta.masks_meta.clear();
    int64_t nselected = in_mask_ids_view[i].shape[0];
    meta.selected_masks = make_cspan(in_mask_ids_view.tensor_data(i),
                                     in_mask_ids_view.tensor_shape(i).num_elements());
    for (auto mask_id : meta.selected_masks) {
      ...
      meta.masks_meta[mask_id].new_mask_id = mask_id;
    }
    if (reindex_mask_) {
      int idx = 0;
      for (auto mask_id : meta.selected_masks)
        meta.masks_meta[mask_id].new_mask_id = idx++;
    }

    int64_t nmasks = in_mask_meta_shape[i][0];
    for (int k = 0, idx = 0; k < nmasks; k++) {
      const auto *mask_data = in_mask_meta_view[i].data + k * 3;
      auto it = meta.masks_meta.find(mask_data[0]);
    ...
    for (int k = 0; k < nselected; k++) {
      ...
      if (mask.start_coord >= mask.end_coord)
        DALI_FAIL(make_string("Selected mask_id ", mask.mask_id,
                              " is not present in the input mask metadata or has no coordinates"));
      out_mask_coords_shape[i][0] += mask.end_coord - mask.start_coord + 1;
    }
  }
...
void SelectMasksCPU::RunImpl(workspace_t<CPUBackend> &ws) {
  ...
  for (int i = 0; i < in_mask_meta_view.num_samples(); i++) {
    ...
    for (int k = 0; k < meta.selected_masks.size(); k++) {
      int mask_id = meta.selected_masks[k];
      ...
      *out_mask_meta_data++ = out_coord_i;  // start coord index
      for (int64_t in_coord_i = mask_meta.start_coord; in_coord_i <= mask_meta.end_coord;
           in_coord_i++, out_coord_i++) {
        out_mask_coords_data[out_coord_i] = in_mask_coords_data[in_coord_i];
      }
      *out_mask_meta_data++ = out_coord_i - 1;  // end coord index
    }
  }
}

DALI_REGISTER_OPERATOR(SelectMasks, SelectMasksCPU, CPU);
