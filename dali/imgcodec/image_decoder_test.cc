 ...
 protected:
  ...
  void TestSingleFormatDecodeSample(const std::string& file_fmt, DALIImageType color_fmt) {
    ...
    try {
      ...
      cv::Mat vref_mat(v.shape[0], v.shape[1], color_fmt == DALI_GRAY ? CV_8UC1 : CV_8UC3, refdata);

      cv::imwrite("/tmp/img_out.bmp", rgb2bgr(v_mat));
      cv::imwrite("/tmp/img_ref.bmp", rgb2bgr(vref_mat));

      cv::Mat diff;
      cv::absdiff(v_mat, vref_mat, diff);
      cv::imwrite("/tmp/img_diff.bmp", rgb2bgr(diff));

      DALI_FAIL("Failure2");
    }
  }

  cv::Mat rgb2bgr(const cv::Mat& img) {    // TODO(janton): Remove this
    cv::Mat bgr;
  ...
  void TestSingleFormatDecodeBatch(const std::string& file_fmt, DALIImageType color_fmt) {
    ...
    for (int i = 0; i < out.view.size(); i++) {
      try {
        ...
        void *refdata = nullptr;
        if (color_fmt == DALI_YCbCr) {
          refdata = samples[i].ref_ycbcr.raw_mutable_data();
        } else if (color_fmt == DALI_GRAY) {
          refdata = samples[i].ref_gray.raw_mutable_data();
        } else {
          refdata = samples[i].ref.raw_mutable_data();
        }
        cv::Mat vref_mat(v.shape[0], v.shape[1], color_fmt == DALI_GRAY ? CV_8UC1 : CV_8UC3,
                         refdata);

        std::stringstream ss1, ss2, ss3;
        ss1 << "/tmp/img_" << i << "_out.bmp";
        ss2 << "/tmp/img_" << i << "_ref.bmp";
        ss3 << "/tmp/img_" << i << "_diff.bmp";
        cv::imwrite(ss1.str(), rgb2bgr(v_mat));
        cv::imwrite(ss2.str(), rgb2bgr(vref_mat));

        cv::Mat diff;
        cv::absdiff(v_mat, vref_mat, diff);
        cv::imwrite(ss3.str(), rgb2bgr(diff));
